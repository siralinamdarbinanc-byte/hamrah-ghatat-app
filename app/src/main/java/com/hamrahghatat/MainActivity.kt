...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ایجاد ویوها به صورت برنامه‌نویسی
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String?): Boolean {
                ...
                adapter.clear()
                adapter.addAll(filtered.map { "${it.name} - ${it.brand}: ${it.price} تومان" })
                return true
            }
        })
    }
    ...
    private fun fetchProducts() {
        ...
        val request = Request.Builder()
            .url("https://price-api-v2.aliinndd2.workers.dev/search?q=")
            .build()

        client.newCall(request).enqueue(object : okhttp3.Callback {
            override fun onFailure(call: okhttp3.Call, e: IOException) {
                runOnUiThread { adapter.add("خطا در اتصال به سرور") }
            }
            ...
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                ...
                try {
                    ...
                    runOnUiThread {
                        ...
                        adapter.clear()
                        adapter.addAll(products.map { "${it.name} - ${it.brand}: ${it.price} تومان" })
                    }
                } catch (e: Exception) {
                    runOnUiThread { adapter.add("خطا در پردازش داده‌ها: ${e.message}") }
                }
            }
        })
    }
